...
const IPHONE_STEPS = [
  <span key={0}>Ouvre <strong>karaf.fr</strong> dans <strong>Safari</strong></span>,
...
const ANDROID_STEPS = [
  <span key={0}>Ouvre <strong>karaf.fr</strong> dans <strong>Chrome</strong></span>,
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-3">
          <div>
            <h2 className="text-base font-semibold" style={{ color: "#1a1a18" }}>
              Installer Karaf
            </h2>
        ...
        <div className="px-6 mb-4">
          <div className="flex gap-1.5 p-1 rounded-xl" style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.06)" }}>
            {([["iphone", "🍎 iPhone"], ["android", "🤖 Android"]] as [Tab, string][]).map(([t, label]) => (
              <button
                ...
                style={{
                  background: tab === t ? "var(--accent, #0f5132)" : "transparent",
                  color: tab === t ? "#fafaf7" : "#888",
                }}
              >
                {label}
              </button>
        ...
        <div className="px-6 pb-6">
              ...
              {(tab === "iphone" ? IPHONE_STEPS : ANDROID_STEPS).map((step, i) => (
                <li key={i} className="flex gap-3 items-start">
                  <span
                    ...
                    style={{ background: "var(--accent, #0f5132)", color: "#fafaf7" }}
                  >
                    {i + 1}
                  </span>
          ...
          <p className="mt-5 text-xs text-center" style={{ color: "#aaa" }}>
            {tab === "iphone"
