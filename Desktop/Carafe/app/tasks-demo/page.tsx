...
export default function TasksDemo() {
  ...
  const totalDone = tasks.filter(t => t.done || taskState[t.id]).length + oneShotTasks.filter(t => taskState[t.id]).length;
  const total = tasks.length + oneShotTasks.length;
  const pct = Math.round((totalDone / total) * 100);

  function validate(id: string) {
    setTaskState(s => ({ ...s, [id]: true }));
  ...
  const styles = {
    ...
    taskRow: (done: boolean, flash: boolean) => ({ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 16px", background: flash ? "rgba(16,185,129,0.08)" : done ? "rgba(16,185,129,0.04)" : "#09090B", borderBottom: "1px solid #1F1F23", transition: "all 0.3s", border: flash ? "1px solid rgba(16,185,129,0.3)" : "none" }) as React.CSSProperties,
    validateBtn: { padding: "6px 12px", borderRadius: 6, background: "rgba(6,182,212,0.1)", color: "#06B6D4", border: "1px solid rgba(6,182,212,0.2)", fontSize: 11, fontWeight: 600, cursor: "pointer", flexShrink: 0 } as React.CSSProperties,
  ...
  const TaskCard = ({ task, isCatchup = false }: { task: typeof tasks[0] | typeof catchupTasks[0]; isCatchup?: boolean }) => {
    ...
    const isDone = fullTask.done || taskState[task.id];
    const flash = justDone.includes(task.id);

    return (
      <div style={styles.taskRow(isDone, flash)}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {isDone && fullTask.by && (
            <p style={{ fontSize: 11, color: "#71717A", margin: "2px 0 0" }}>{fullTask.by} · {fullTask.at}</p>
          )}
  ...
  const SectionBlock = ({ title, icon, items, cat }: { title: string; icon: string; items: typeof tasks; cat: string }) => {
    const done = items.filter(t => t.done || taskState[t.id]).length;
    return (
    ...
    <div style={styles.page}>
      ...
      {oneShotTasks.length > 0 && (
        <div style={{ ...styles.section, marginTop: 16, border: "1px solid #27272A", borderRadius: 12, overflow: "hidden" }}>
          ...
          {oneShotTasks.map(t => (
            <div key={t.id} style={styles.taskRow(!!taskState[t.id], justDone.includes(t.id))}>
              <span style={{ color: taskState[t.id] ? "#10B981" : "#3F3F46", fontSize: 18, marginTop: 2 }}>
                {taskState[t.id] ? "✓" : "○"}
              </span>
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 13, fontWeight: 600, margin: 0, color: taskState[t.id] ? "#71717A" : "#FAFAFA", textDecoration: taskState[t.id] ? "line-through" : "none" }}>{t.title}</p>
          ...
          <div style={styles.modalBox} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
              <div>
                ...
                <p style={{ fontSize: 12, color: "#71717A", margin: "4px 0 0" }}>{tasks.find(t => t.id === modal)?.title ?? oneShotTasks.find(t => t.id === modal)?.title}</p>
              </div>
